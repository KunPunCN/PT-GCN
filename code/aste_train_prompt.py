...
from model.ptgcn_model import BDTFModel
...
class ASTE(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        outputs = self(**batch)
        # loss = outputs['table_loss']
        # if self.current_epoch>3:
        loss = outputs['table_loss'] + outputs['pair_loss']
    ...
    def validation_step(self, batch, batch_idx):
        with torch.no_grad():
            outputs = self(**batch)
        loss = outputs['table_loss'] + outputs['pair_loss']  # + outputs['em_loss']
